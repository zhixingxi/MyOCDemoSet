...
open class RichEditorView: UIView, UIScrollViewDelegate, UIWebViewDelegate, UIGestureRecognizerDelegate {
    ...
    @objc open var isScrollEnabled: Bool = true {
        didSet {
    ...
    @objc open var isEditingEnabled: Bool {
    ...
    public func setMyFontSize(_ size: Int) {
        runJS("RE.setMyFontSize(\(size));")
    }
    public func getMyFontSize() -> Int {
       let str = runJS("RE.getMyFontSize();")
        return Int(str) ?? 4
    }
    ...
    @objc public func insertImage(_ url: String, alt: String) {
        runJS("RE.prepareInsert();")
